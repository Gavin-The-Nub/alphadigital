      ...
      hits: [
        {
          _key: "1",
          document: {
            _slugPath: "/articles/article-1",
            _title: "Article 1",
            body: "This is the body of Article 1. It contains some text.",
            description: "Description for Article 1.",
            categories: ["Category A", "Category B"],
            authors: [
              { _id: "author1", _title: "Author 1" },
              { _id: "author2", _title: "Author 2" },
            ],
          },
          highlights: [
            { fieldPath: "authors.0._id", snippet: "Author 1" },
            { fieldPath: "authors.1._id", snippet: "Author 2" },
          ],
        },
        {
          _key: "2",
          document: {
            _slugPath: "/articles/article-2",
            _title: "Article 2",
            body: "This is the body of Article 2. It contains some text.",
            description: "Description for Article 2.",
            categories: ["Category X"],
            authors: [{ _id: "author1", _title: "Author 1" }],
          },
          highlights: [{ fieldPath: "authors.0._id", snippet: "Author 1" }],
        },
      ],
    },
  };
...
function HitList({ hits }: { hits: any[] }) {
  return (
    <SearchBox.HitList className="space-y-2">
      {hits.map((hit) => {
        const pathname = hit.document._slugPath ?? "";

        const field = hit.document.authors;
        ...
        for (const h of hit.highlights) {
          if (h.fieldPath.startsWith("authors")) {
            ...
            if (!index) continue;
            const id = hit.document._getField(`authors.${index}._id`);
